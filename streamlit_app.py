...
if menu == "📝 每日数据录入":
    ...
    if not df.empty:
        ...
        # 🌟 顶部增加：导出按钮区
        with st.expander("📑 数据导出与管理"):
            st.info("您可以将库中所有历史数据导出为 Excel 文件。")
            excel_data = convert_df_to_excel(df)
            st.download_button(
                label="📥 点击下载完整业务 Excel 表",
                data=excel_data,
                file_name=f"影像科业务数据_{datetime.now().strftime('%Y%m%d')}.xlsx",
        ...
        st.markdown("---")
        if st.checkbox("查看最新 7 条原始记录"):
            st.dataframe(df.head(7), use_container_width=True)
    else:
        st.warning("库内暂无数据")

if st.sidebar.button("🔄 强制同步云端数据"):
    st.cache_data.clear()
    st.rerun()
